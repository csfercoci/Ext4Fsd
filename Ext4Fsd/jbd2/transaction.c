				   ...
				   int rsv_blocks)
{
	 ...
	 */
	if (jbd2_log_space_left(journal) < (unsigned long)jbd2_space_needed(journal)) {
		atomic_sub(total, &t->t_outstanding_credits);
		read_unlock(&journal->j_state_lock);
		jbd2_might_wait_for_commit(journal);
...
int jbd2_journal_extend(handle_t *handle, int nblocks)
{
	...
	if ((unsigned long)(wanted + (wanted >> JBD2_CONTROL_BLOCKS_SHIFT)) >
	    jbd2_log_space_left(journal)) {
		jbd_debug(3, "denied handle %p %d blocks: "
...
{
	printk(KERN_WARNING
	       "JBD2: Spotted dirty metadata buffer (dev = %p, blocknr = %llu). "
...
static void jbd2_freeze_jh_data(struct journal_head *jh)
{
	...
	page = bh->b_page;
	offset = (unsigned long)((ULONG_PTR)bh->b_data & (PAGE_SIZE - 1));
